...
###### validacao de JSON ###########
require_once("jsonlint/utils/utilsJson.php");
...
//	requires
    ...
    require_once('includes/formResposta.class.php');
    require_once('includes/formMetaResposta.class.php');
    ...
    // Clientes
    require_once('includes/clientes/integracao.class.php');
...
//	require_once('/includes/geo.php');
	...
	switch($act){
        ...
        case 'initRastreamento':
                    ...
                    if($u = $user->getUserByToken($_REQUEST['id'],$_REQUEST['token'])){
        ...
        case 'kpi':
            ...
            if($user->getUserByTokenPainel($_REQUEST['idUsuario'],$_REQUEST['token'])){
                if($r = $kpi->montaKpi($userJwt->empresa,$_REQUEST['pullTracker']['mensal'],$_REQUEST['grupoAtivo'])){
        ...
        case 'kpi-visitas-dias':
            ...
            // if($user->getUserByTokenPainel($_REQUEST['idUsuario'],$_REQUEST['token'])){
                if($r = $kpi->montaKpiPorAncora($userJwt->empresa,
        ...
        case 'relatorioExtratoVisitasCielo':
            ...
            break;
        case 'update_tarefa_meta':
            if (!$_REQUEST['tarefa'] || !$_REQUEST['meta']) {
                echo json_encode([
                    'status' => 'error',
                    'mensagem' => 'Campos tarefas e meta obrigatórios'
                ]);
                http_response_code(400);
                die();
            }
            foreach ($_REQUEST['meta'] as $meta) {
                $tarefas->update_tarefa_meta($_REQUEST['tarefa']['id'], $meta['meta_key'] ,str_replace("'","\'", $meta['meta_value'] ));
            }
            // registra um update para sincronização
            $tarefas->execute('update tarefas set updated_at = now() where id = '. $_REQUEST['tarefa']['id']);

            echo json_encode([
                'status' => 'ok',
                'mensagem' => 'Metadados atualizado com sucesso!'
            ]);
            break;
        
        case 'sincronizarTarefasWhirlpool':
        
            $tarefaClass = new tarefas();
            $rota = new rota();
            $tarefasLog = new TarefasLog();

            $retorno['status'] = 'ok';
            $retorno['mensagem'] = 'Nenhuma tarefa encontrada';
            $retorno['tarefasSincronizadas']  = [];
            $retorno['idTarefasSincronizadas']  = [];
            $retorno['tarefa_log_sincronizada']  = [];

            $tarefas = isset($_POST['tarefas']) ? json_decode($_POST['tarefas']) : false;
            $versaoTarefas = isset($_POST['relacaoTarefaVersao']) ? json_decode($_POST['relacaoTarefaVersao']) : false;
            $ListatarefasLog = isset($_POST['tarefa_log']) ? json_decode($_POST['tarefa_log']) : false;

            if ($tarefas) {
                // salva todas as tarefas no banco de dados
                foreach ($tarefas as $tarefa) {
                    $newTarefa = $tarefaClass->save($tarefa);
                    $retorno['tarefasSincronizadas'][] = $newTarefa;
                    $retorno['idTarefasSincronizadas'][] = $newTarefa->id;
                }
                $retorno['status'] = 'ok';
            ...
            if($ListatarefasLog) {
                foreach ($ListatarefasLog as $log) {
                    if($newTarefa = $tarefasLog->save($log)) {
                        $retorno['tarefa_log_sincronizada'][] = $newTarefa;
                    }
                }
            }
            
            if($_REQUEST['pullTracker']['data']!=''){
                if($r = $rota->loadRotaForUserWhirlpool($_REQUEST['pullTracker']['data'], $retorno['idTarefasSincronizadas'], $versaoTarefas )){
                    $retorno['rota'] = $r;
                    $retorno['tarefasDeletadas'] = $r['tarefasDeletadas'];
                }
            }else{
                if($r = $rota->loadRotaForUserWhirlpool(null, $retorno['idTarefasSincronizadas'], $versaoTarefas )){
                    $retorno['rota'] = $r;
                    $retorno['tarefasDeletadas'] = $r['tarefasDeletadas'];
                }
            }
            ...
            /**
             * bugfix/PR-1140 - Distribuição das OS's do dia seguinte aparecendo botão de solicitar aprovação de rota
             * 
             * Verificando se a Rota Bloqueada pertence ao dia atual
             */
            $dia = $_REQUEST['pullTracker']['data'];
            $dataFormatada = $dia[6].$dia[7].$dia[8].$dia[9].'-'.$dia[3].$dia[4].'-'.$dia[0].$dia[1];
            $rotaBloqueada = $rota->verificaRotaBloqueada($retorno['user']['route']->id, $dataFormatada);
            if (!$rotaBloqueada) {
                $retorno['user']['route']->status = 0;
            } else if($rotaBloqueada && 
                        $retorno['user']['route']->status == 0){
                $retorno['user']['route']->status = 1;
            } else if($rotaBloqueada && 
                        $retorno['user']['route']->status == 2 &&
                        date('Y-m-d', strtotime($retorno['user']['route']->data)) < date('Y-m-d', strtotime($dataFormatada)) ){
                $retorno['user']['route']->status = 1;
            }
        ...
        case 'loginCompleto':
                ...
                if($u = $user->getUserByToken($userId,$token)){
                    ...
                    if(isset($_REQUEST['APP_VERSION'])){
                        $user->update_user_meta($u['id'],'appVersion',$_REQUEST['APP_VERSION']);
                    }
                  ...
                  //  $retorno['meta']['meta_empresa']['mostrarBotoesExtras'] = $empresas->get_empresa_meta($retorno['user']['empresa'],'mostrarBotoesExtras');
                    ...
                    /**
                     * WHIRLPOOL - Caso o valor passado seja 2000 ele setará um valor maior para o app
                     */
                    if(intval($retorno['meta']['meta_user']['DistanciaChegada']['valor']) >= 2000){
                        $retorno['meta']['meta_user']['DistanciaChegada']['valor'] = 12713 * 1000; // Diametro da terra;
                    }
        ...
        case 'atualizaGruposListItem':
            $form = new formulario();
            $form->normalizeItensGrupo($_REQUEST['id'],$_REQUEST['grupos']);
            $retorno['status'] = 'true';
...
//                }
        ...
        case 'carregarEmpresaMetas':
            ...
            if($metas = $empresas->carregarEmpresaMetas((in_array($userJwt->empresa,['7','8']) ? $_REQUEST['empresa'] : $userJwt->empresa ))){
                 $retorno['status'] = 'ok';
        ...
        case 'cadastrarMetaEmpresa':
            ...
            if($empresas->update_empresa_meta($userJwt->empresa,$_REQUEST['metakey'],$_REQUEST['metavalue'])){

                if($_REQUEST['metakey'] == 'bloqueioCriacaoTarefaApp'){
                    $retorno['ids_usuarios_updatiados'] = $empresas->bloqueiaCriacaoTarefaTodosUsuarios($userJwt->empresa, $_REQUEST['metavalue']);
                }
		...
		case 'responderChat':
			//	mail('glaysonramos@gmail.com','teste',print_r($_POST,true));
                    $geo = new geo();
					$mensagem = $geo->responderChat($_POST['chatUser'], $_POST['chatAdmin'], $_POST['mensagem'], false, 'pt-BR', $_POST['id']);
            ...
            $db->desconecta();
				die();
            break;
        case 'responderChatAPP':
            $geo = new geo();
            $date = new DateTime();
            $entregue = $date->getTimestamp();

            foreach($_REQUEST['mensagens'] as $mensagem){
                $mensagem['enviada'] = $entregue;

                $mensagemRetorno = $geo->responderChatAPP($mensagem,$_REQUEST['criada'],'pt-BR');

                if($mensagemRetorno['status'] == 'OK'){
                    $notificacao = new notificacao();
                    if(($_POST['chatAdmin'] == $_POST['chatUser'])||($_REQUEST['pullTracker']['usoPeloPainel'] == 'false')){
                        do_action('not_post_mensagem', [1,$mensagem['mensagem'],$_POST['chatUser'],'0','']);
                    }
                }
                $retorno['mensagensSincronizadas'][] = $mensagemRetorno;
                $retorno['status'] = 'OK';
                $retorno['message'] = 'OK';
            }
        ...
        case 'loadchatSince':
            $geo = new geo();
            $retorno = array();
            $retorno = $geo->loadchatSince($_REQUEST['pullTracker']['user'],$_REQUEST['pullTracker']['last']);
            echo json_encode($retorno);
            break;
		case 'entregarChat':
					$geo = new geo();
...
// send email
		...
		case 'criarUser':
				...
				if($_REQUEST['email']!=''){
					if($d = mysqli_fetch_object($user->execute('SELECT id from users where  email = \''.$_REQUEST['email'].'\' AND ativo = 1'))){
                        if($d->id) {
                            $retorno['status'] = 'false';
                            $retorno['mensagem'][] = 'Este e-mail já está cadastrado em nosso sistema.';
                        }
				...
				if($_REQUEST['editarId']!='' || !$d){
					...
					if($_REQUEST['veiculo']==''){
						$retorno['status'] = 'false';
						$retorno['mensagem'][] = 'Por favor, escolha um tipo de veículo';
					}
                    ...
                    if($_REQUEST['email']=='' && $_REQUEST['isAdmin']=='on'){

                        $retorno['status'] = 'false';
                        $retorno['mensagem'][] = 'Agentes administradores precisam de um e-mail para realizar o login.';

                    }
                    if($_REQUEST['email']=='' && $_REQUEST['emailBV']=='on'){

                        $retorno['status'] = 'false';
                        $retorno['mensagem'][] = 'Por favor, informe um email.';
					...
					if($retorno['status'] == 'ok'){
						...
						if($d || $_REQUEST['editarId']){
							...
							$user->updateUser($user->gerarUserCode($id),'code',$id);
						...
						if($id){
							...
							if($_REQUEST['isAdmin']== 'on'){
								$user->updateUser(1,'isAdmin',$id);
							}else{
								$user->updateUser(0,'isAdmin',$id);
							}

							$user->updateUser($_REQUEST['cellPhone'],'celular',$id);
                            ...
                            */
							...
							$user->updateUser($_REQUEST['veiculo'],'veiculo',$id);

							$token = md5($id.date('YMD'));

							$user->updateUser($token,'token',$id);
    ...
    case 'ativarAgenteDesativado':
        ...
        case 'loadAgenteComMatricula': 

            $user = new user();
            $retorno = [];
            $retorno['status'] = 'false';
            $retorno['mensagem'] = '';
            if($us = $user->loadAgenteComMatricula($userJwt->empresa,$_REQUEST['pullTracker']['dataAtiva'], $_REQUEST['pullTracker']['grupoAtivo'],$_REQUEST['pullTracker']['user'])){
                $retorno['status'] = 'ok';
                $retorno['users'] = $us;
            }
            $retorno['grupos'] = false;
        ...
        case 'loadAgentesDeOutraEmpresa':
            ...
            case 'loadListaAgentesDeOutraEmpresaComMatricula':

            $user = new user();

            $retorno['status'] = 'false';
            $retorno['mensagem'] = '';
            if($us = $user->loadListaAgentesDeOutraEmpresaComMatricula($_REQUEST['pullTracker']['empresa'])){
                $retorno['status'] = 'ok';
                $retorno['users'] = $us;
                $retorno['mensagem'] = 'Usuarios Encontrados!';
            }
		...
		case 'loadUserTasks':
				...
				if($retorno['tarefas'] = $rota->loadUserTasks($_REQUEST['pullTracker']['rota'],$_REQUEST['pullTracker']['data'])){
					$retorno['status'] = 'ok';
					$retorno['mensagem'] = '';
				}
...
*/
        ...
        case 'reagendarTarefa':
            ...
            if($resultado){
                ...
                $retorno['mensagem'] = mysqli_error(db::$conn);
                $retorno['status'] = 'ERRO';
            }
        ...
        case 'pegaUsuariosEmTarefaInativos':
            ...
            echo('ERRO -> '.mysqli_error(db::$conn));
            }
            $numeroDeUsuario = 0;
        ...
        if($_REQUEST['pullTracker']['editarId']!=''){
            if(empty($_REQUEST['senha'])){
                if($empresas->verificarEmpresa($_REQUEST['nome'],$_REQUEST['isAdmin'],$status,$_REQUEST['email'],$_REQUEST['criado'],$_REQUEST['maxUsers'],$_REQUEST['ativo'],$_REQUEST['pullTracker']['editarId'],$_REQUEST['menus'])){
                        $retorno['status'] = 'ok';
                ...
                if($empresas->atualizarEmpresa($_REQUEST['nome'],$_REQUEST['isAdmin'],$status,$_REQUEST['email'],$senha,$token,$_REQUEST['criado'],$_REQUEST['maxUsers'],$_REQUEST['ativo'],$_REQUEST['pullTracker']['editarId'],$_REQUEST['menus'])){
                        $retorno['status'] = 'ok';
            ...
            if($idEmpresa = $empresas->cadastrarEmpresa($_REQUEST['nome'],$_REQUEST['isAdmin'],$status,$_REQUEST['email'],$senha,$token,$_REQUEST['criado'],$_REQUEST['maxUsers'],$_REQUEST['ativo'],$_REQUEST['menus'])){
                $retorno['status'] = 'ok';
        ...
        if($_REQUEST['metaDados'] != '') {
            $arrayMeta = $_REQUEST['metaDados'];

            $idEmpresa = ( (isset($_REQUEST['pullTracker']['editarId']) && ($_REQUEST['pullTracker']['editarId'] != "")) ? $empresas->buscarEmpresa($_REQUEST['pullTracker']['editarId'])['empresa'] : $idEmpresa);

            foreach ($arrayMeta as $key => $value) {
                $empresas->update_empresa_meta($idEmpresa,$key,$value);
            }
        }
        ...
        case 'ativarEmpresa':
            ...
            $empresas->ativarEmpresa($_REQUEST['pullTracker']['id']);
            $retorno['status'] = 'ok';
        ...
        case 'desativarEmpresa':
            ...
            $empresas->desativarEmpresa($_REQUEST['dataDesativar'], $_REQUEST['pullTracker']['id']);
            $retorno['status'] = 'ok';
        ...
        case 'relatoriosServerSideHorasRastreadas':
            ...
            case 'buscaCategoriasOS':
                ...
                $user = new user();
                    ...
                    $whirlpoll = new Whirlpool();

                    $retorno['respostas'] = json_encode($whirlpoll->buscaCategoriasOS());
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Categorias Encontradas';

                echo json_encode($retorno);

                break;

            case 'buscaOSRoteirizacao':

                $init = time();
                $retorno['status'] = 'false';
                $retorno['mensagem'] = 'Nenhuma OS Encontrada';
                $retorno['respostas'] = '';
                $whirlpoll = new Whirlpool();
                $dataAgendamentoInicial = $_REQUEST['pullTracker']['dataAgendamentoInicial'];
                $dataAgendamentoFinal = $_REQUEST['pullTracker']['dataAgendamentoFinal'];
                $periodo = $_REQUEST['pullTracker']['period'];
                $dados = $whirlpoll->buscaOSRoteirizacao($periodo, $dataAgendamentoInicial, $dataAgendamentoFinal);
                $fim = time();
                $retorno['respostas'] = $dados['os'];
                $retorno['timeApi'] = $dados['timeApi'];
                $retorno['timeTotal'] = $fim - $init;
                $retorno['timeTracker'] = $fim - $init - $dados['timeApi'];
                $retorno['status'] = 'ok';
                $retorno['mensagem'] = 'OS Encontradas';
            ...
            case 'salvarLogOS':
                ...
                $idLog = $whirlpoll->salvarLogOS($_REQUEST['pullTracker']['nOS'],$_REQUEST['pullTracker']['mensagem'],$_REQUEST['pullTracker']['data']);

                if($idLog != ''){

                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Log salvo com sucesso';
                }
            ...
            case 'criarTarefaPelaOS': 

                $retorno['status'] = 'false';
                $retorno['mensagem'] = 'Erro ao distribuir';
                $retorno['respostas'] = '';

                $whirlpoll = new Whirlpool();
            
                if($retorno = $whirlpoll->criarTarefaPelaOS(json_decode($_REQUEST['pullTracker']['listaOS'], true))) {

                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Tarefas criadas com sucesso';
                }
            ...
            case 'notificaAgentesDistribuicao': 

                $retorno['status'] = 'false';
                $retorno['mensagem'] = 'Erro ao notificar';
                $retorno['respostas'] = '';

                $whirlpoll = new Whirlpool();
            
                $retorno = $whirlpoll->notificaAgentesDistribuicao($_REQUEST['listaAgentesParaNotificarApp']);
                $retorno['status'] = 'ok';
                $retorno['mensagem'] = 'Notificações enviadas';

                echo json_encode($retorno);

                break;   

            case 'whirpool_get_listas_empresas':

                if($users = $whirlpoll->getListasEmpresas()){
                    $retorno['users'] = $users;
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Empresas encontradas!';
                }else{
            ...
            case 'whirpool_relatorio_os':

                if($respostas = $whirlpoll->getRelatorioOS($_REQUEST['datainicial'],$_REQUEST['datafinal'],$userJwt->empresa)){
            ...
            case 'whirpool_relatorio_comparacao_os':

                if($respostas = $whirlpoll->getRelatorioComparacaoOS((in_array($userJwt->empresa,['7','8']) ? $_REQUEST['empresa'] : $userJwt->empresa),$_REQUEST['filtroData'])){
                    if($_REQUEST['empresa'] === '0') {
                        $retorno['mensagem'] = 'Selecione pelo menos uma AT!';
                        $retorno['status'] = 'ERRO';
                    }
                    else if ($respostas == -1) {
                        $retorno['mensagem'] = 'Nenhuma Os foi distribuida nessa data!';
                        $retorno['status'] = 'ERRO';
                    } else {
                        $retorno['respostas'] = $respostas['unitarios'];
                        $retorno['consolidados'] = $respostas['consolidados'];
                        $retorno['status'] = 'ok';
                        $retorno['mensagem'] = 'OS encontradas!';
                    }
            ...
            case 'whirpool_acompanhamento_os':

                if($respostas = $whirlpoll->getRelatorioAcompanhamentoOS((in_array($userJwt->empresa,['7','8']) ? $_REQUEST['empresa'] : $userJwt->empresa),$_REQUEST['filtroDataInicial'],$_REQUEST['filtroDataFinal'],$_REQUEST['nos'])){
            ...
            case 'whirpool_get_rotas_para_aprovar':

                if($users = $whirlpoll->getRotasParaAprovar($userJwt->empresa)){
                    $retorno['users'] = $users;
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Solicitação de aprovação rotas encontradas!';
                }else{
                    $retorno['mensagem'] = 'Nenhuma rota está aguardando confirmação';
                }
                echo json_encode($retorno);
                break;

            case 'whirpool_get_rota_tarefas_bloqueadas':

                if($tarefas = $whirlpoll->getRotaBloqueada($_REQUEST['rota'])){
                    $retorno['tarefas'] = $tarefas;
                    $retorno['status'] = 'ok';
                }else{
                    $retorno['mensagem'] = 'Nenhuma tarefa aguardando aprovação';
                }
                echo json_encode($retorno);
                break;

            case 'whirpool_get_relatorio_pedido_pecas':

                if($tarefas = $whirlpoll->getRelatorioPedidoPecas($userJwt->empresa,$_REQUEST['pullTracker']['dataAtiva'])){
                    $retorno['tarefas'] = $tarefas;
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'OS encontradas!';
                }else{
            ...
            case 'whirpool_regeneratePreResp' :
                ...
                echo $whirlpoll->regeneratePreResp($_REQUEST['tarefa']);
                die();
            ...
            case 'whirpool_get_dados':
                ...
                break;
            case 'whirpool_get_dados_by_modelo':
    
                ini_set("memory_limit", "-1");
                if(!$_REQUEST['modelo'] || !$_REQUEST['applianceLineCode']){

                    echo json_encode([
                        "status" => "false",
                        "mensagem" => "parametro modelo e applianceLineCode obrigatório"
                    ]);
                    die();
                }
                

                $retorno = $whirlpoll->getDadosModelo($_REQUEST['modelo'],$_REQUEST['applianceLineCode']);
                echo json_encode($retorno);
                die();

                break;
            case 'whirpool_get_relatorio_financeiro':
                
                $retorno['status'] = 'false';
                $retorno['mensagem'] = 'Nenhum dado encontrado!';

                if($dados = $whirlpoll->getrelatorioFinanceiro($_REQUEST['data'],$_REQUEST['empresa'])) {
                    
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Dados encontrados com sucesso';
                    $retorno['tarefas'] = $dados;
                };
                //$retorno = $whirlpoll->post_save_tarefa_concluida("45506597",2027);
                echo json_encode($retorno);
                die();
            ...
            case 'whirpool_get_dados_from_material':
                if(!$_REQUEST['material']){
                    echo json_encode([
                        "status" => "false",
                        "mensagem" => "parametro material obrigatório"
                    ]);
                    die();
                }
                $retorno = $whirlpoll->getDadosOSFromMaterial($_REQUEST['material']);
                echo json_encode($retorno);
                break;
                
            case 'whirpool_request_aprovar_rota':

                $retorno = $whirlpoll->requestAprovarRota($userJwt);

                //Notificar Agente Tracker PAINEL
                $texto = 'Foi solicitada uma aprovação de rota!';
                $notificacao = new notificacao();
                $notificacao->criarNotificacao(16,$texto,$userJwt->id,$userJwt->id);
            ...
            case 'whirpool_list_aprovar_rota':

                $retorno = $whirlpoll->listAprovarRota($_POST['rota']);
                echo json_encode($retorno);

                break;
            
            case 'whirpool_aprovar_rota':

                if(!$_POST['rota']){
                    echo json_encode([
                        "status" => "false",
                        "mensagem" => "parametro rota obrigatório"
                    ]);
                    die();
                }

                if($rota = $whirlpoll->aprovarRota($_REQUEST['rota'])){
                    $retorno['rota'] = $rota;
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Rota Aprovada com sucesso';

                    try {
                        $rotaObject = new rota();
                        $idParaNotificar = mysqli_fetch_object($whirlpoll->getOne('*',$_REQUEST['rota'],'rotas'))->user;
                        $rotaObject->notificar($idParaNotificar,'Rota aprovada!','A sua sugestão de rota foi aprovada!','tarefa');
                        $retorno['notificacao'] = 'Notificação enviada para o Técnico';
                    } catch ( Exception $e ) {
                        $retorno['notificacao'] = 'Erro ao enviar notificação para o Técnico';
                    }
                }else{
                    $retorno['mensagem'] = 'Erro na aprovação da rota';
                }
                echo json_encode($retorno);

                break;
            case 'whirpool_reprovar_rota':
                ...
                if($rota = $whirlpoll->reprovarRota($_REQUEST['rota'])){
                    $retorno['rota'] = $rota;
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Rota reprovada com sucesso';
                }else{
                    $retorno['mensagem'] = 'Erro na reprovação da rota';
                }
                echo json_encode($retorno);
            ...
            case 'whirpool_send_OS':
                ...
                $retorno = $utilsJson->arrange_json($whirlpoll->post_save_tarefa_concluida($_REQUEST['tarefa'] ,$_REQUEST['user']));
                print_r($retorno);
                die();
            ...
            case 'whirpool_print_os':
                if(is_file('/var/simplesamlphp/lib/_autoload.php')) {
                    require_once('/var/simplesamlphp/lib/_autoload.php');
                    $as = new \SimpleSAML\Auth\Simple('default-sp');
                    $as->requireAuth([
                        'ReturnTo' => 'https://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],
                        'KeepPost' => FALSE
                    ]);
                    ...
                    $attributes = [
                        'emailAddress' => [
                            'tiago_a_ramos_trackerup@whirlpool.com'
                        ],
                        'user ID' => [
                            'RAMOSTA'
                        ]
                ...
                case 'whirpool_load_lista_status_atendimento':

                if($status = $whirlpoll->getlistaStatus()){
                    $retorno['listastatus'] = $status;
                    $retorno['status'] = 'ok';
                    $retorno['mensagem'] = 'Lista de Status com sucesso';
                }else{
                    $retorno['mensagem'] = 'Erro na Lista de Status';
                }
                echo json_encode($retorno);
            break;
            ...
            case 'post_save_tarefa_concluida':


                echo json_encode($whirlpoll->post_save_tarefa_concluida($_REQUEST['tarefa'],$_REQUEST['user']));
                break;
            ...
            case 'teste':
                // $notificacao = new notificacao();
                // $notificacao->criarNotificacao(15,'Cliente XXXXXX informa indisponibilidade na OS XXXXXX. Tecnico XXXXX, Telefone Cliente: (XX) XXXXX-XXXX',21,$data['idServer']);
                // $whp = new Whirlpool();
                // $tarefa = new Tarefas();
                // $metaDadosTarefa = $tarefa->get_tarefa_meta(1280);
                // $dados_os = json_decode($metaDadosTarefa["__dados_os"]);
                // $whp->sendEmailCliente(1280,$dados_os,'joao.paulo@trackerup.com.br');
            ...
            case 'indicadoresTarefa':
                ...
                $statusEspecifico = $_REQUEST['statusEspecifico'];
                $empresa = $userJwt->empresa;
                $tipoFiltroData = $_REQUEST['tipoFiltroDeData'];
                ...
                $tarefa = new indicadoresTarefa();
                if($indicadores = $tarefa->get_indicadores_tarefa($gruposParticipantes, $empresa, $dataInicio, $dataFim, $tipoFiltroData, $grupoEspecifico, $agenteEspecifico, $statusEspecifico, $exportar)){
                    $retorno = $indicadores;
                }
...
// ');
            ...
            case 'criarCategoriaQRCode':
                ...
                if($id = $qrcode->categoriaExiste($_REQUEST['nome'],$userJwt->empresa) &&  $_REQUEST['editarId']==''){
                    $retorno['status'] = 'false';
                    $retorno['mensagem'] = 'Esta categoria já existe';
                }
                else{
